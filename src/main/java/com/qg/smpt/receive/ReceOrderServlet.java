...
//        }
		...
		if (printers == null) {
			...
			if (ShareMem.userIdMap.get(userId) == null) {
				...
				LOGGER.log(Level.DEBUG, "已将商家信息 id:[{0}], name [{1}], address [{2}], store [{3}] 放入内存中", user.getId(),
						user.getUserName(), user.getUserAddress(), user.getUserStore());
		...
		if (printers != null && printers.size() > 0) {
			if (ShareMem.priBufferMapList != null) {
				...
				LOGGER.log(Level.DEBUG, "当前打印机缓存批次订单队列长度：[{0}], 获取第[{1}] 个批次订单", bOrdersList.size(),
						bOrdersList.size());
				// 获取容器中最后一个批次订单
				BulkOrder bOrders = bOrdersList.get(bOrdersList.size() - 1);

				LOGGER.log(Level.DEBUG, "订单数据转化打印机发送数据");
				BOrder bOrder = order.orderToBOrder((short) printer.getCurrentBulk(),
						(short) bOrders.getbOrders().size());
				...
				if (bOrders.getDataSize() + bOrder.size > Constants.MAX_TRANSFER_SIZE) {
					...
					if (printer.isCanAccept()) {
						...
						LOGGER.log(Level.INFO, "获取打印机 [{0}] 已睡眠printerProcessor 线程 [{1}], 唤醒该线程", printer.getId(),
								processor.getId());
						processor.notifyOK();
					} else {
					...
					LOGGER.log(Level.DEBUG,
							"不满足唤醒打印机 [{0}] 已睡眠线程的条件, 打印机缓冲队列 [{1}]，"
									+ "批次订单数 [{2}], 最后批次订单号 [{3}], 最后批次订单容量 [{4}] byte",
							printer.getId(), bOrdersList.size(), bOrders.getId(), bOrders.getDataSize());
				}
			}
		}
